...
class Ftp extends AbstractFtpAdapter
{
    ...
    public function connect()
    {
	    ...
	    set_error_handler(function($errno, $errstr) { throw new ConnectionRuntimeException('Could not connect to host: ' . $this->getHost() . ', port:' . $this->getPort() . '<br/><br/>Reason:<br/>' .$errstr. '<br/><br/>The FTP server address is probably incorrect.'); });
    	if ($this->ssl) {
    ...
    protected function login()
    {
        ...
        if ( ! $isLoggedIn) {
            ...
            throw new ConnectionRuntimeException(
                ...
                ) . ', username: ' . $this->getUsername() .'<br/><br/>The username or password is probably incorrect.'
            );
        }
    }
