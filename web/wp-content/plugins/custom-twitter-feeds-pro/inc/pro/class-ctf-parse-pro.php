...
class CTF_Parse_Pro
{
    ...
    public static function get_feed_classes( $feed_options, $check_for_duplicates ) {
    ...
    public static function get_generic_header_url ( $data ) {
        $hashtags = ! empty( $data['feed_term'] ) ? explode(" OR ", $data['feed_term']) : '';
        if ( $data['type'] === 'search' || $data['type'] === 'hashtag' ) {
