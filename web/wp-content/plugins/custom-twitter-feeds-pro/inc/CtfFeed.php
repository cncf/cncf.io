...
class CtfFeed
{
    ...
    public function setTweetsToRetrieve()
    {
        ...
        if ( $this->num_needed_input < 1 ) {
            ...
            if ( $this->feed_options['num'] < 10 ) {
	            $count = max( round( $this->feed_options['num'] * $this->feed_options['multiplier'] * 1.6 ), $min_tweets_to_retrieve );
            } elseif ( $this->feed_options['num'] < 30 ) {
	            $count = round( $this->feed_options['num'] * $this->feed_options['multiplier'] * 1.2 );
            } else {
	            $count = round( $this->feed_options['num'] * $this->feed_options['multiplier'] );
            }
        } else {
    ...
    public function getFeedOpeningHtml()
    {
        ...
        $ctf_feed_html .= '<div id="ctf" class="' . $ctf_feed_classes . '" style="' . $feed_options['width'] . $feed_options['height'] . $feed_options['bgcolor'] . '" data-ctfshortcode="' . $this->getShortCodeJSON() . '"' .$ctf_data_disablelinks . $ctf_data_linktextcolor . ' data-ctfneeded="'. $ctf_data_needed . '"' . $ctf_data_other . '>';
