...
class CTF_oEmbeds {
	 ...
	 * @var string
	 */
	const SLUG = 'ctf-oembeds-manager';
	...
	function register_menu() {
       ...
       $oembeds_manager = add_submenu_page(
           ...
           __( 'oEmbeds', 'custom-twitter-feeds' ),
           __( 'oEmbeds', 'custom-twitter-feeds' ),
           $cap,
           self::SLUG,
	...
	public function disable_instagram_oembed_from_instagram () {
		\TwitterFeed\Builder\CTF_Feed_Builder::check_privilege( 'nonce' );

		$oembed_settings = get_option( 'ctf_oembed_token', array() );
		$oembed_settings['access_token'] = '';
		$oembed_settings['disabled'] = true;
		update_option( 'ctf_oembed_token', $oembed_settings );
    ...
    public function oembeds_enqueue_admin_scripts(){
		...
		if ( ! 'instagram-feed_page_ctf-oembeds-manager' === $screen->id ) {
            return;
		}

		wp_enqueue_style(
			'oembeds-style',
			CTF_PLUGIN_URL . 'admin/assets/css/oembeds.css',
			false,
			CTF_VERSION
		);
		...
		wp_enqueue_script(
			'oembeds-app',
			CTF_PLUGIN_URL.'admin/assets/js/oembeds.js',
			null,
			CTF_VERSION,
			true
		);

		$ctf_oembends = $this->statuses_and_info();
		$ctf_oembends['nonce'] = wp_create_nonce( 'ctf-admin' );
	...
	public function statuses_and_info() {
		$return = array(
			...
			'genericText'       => array(
				...
				'title' => __( 'oEmbeds', 'custom-twitter-feeds' ),
				'description' => __( 'Use Smash Balloon to power any Instagram or Instagram oEmbeds across your site. Just click the button below and we\'ll do the rest.                ', 'custom-twitter-feeds' ),
				'instagramOEmbeds' => __( 'Instagram oEmbeds are currently not being handled by Smash Balloon', 'custom-twitter-feeds' ),
				'instagramOEmbedsEnabled' => __( 'Instagram oEmbeds are turned on', 'custom-twitter-feeds' ),
				'facebookOEmbeds' => __( 'Facebook oEmbeds are currently not being handled by Smash Balloon', 'custom-twitter-feeds' ),
				'facebookOEmbedsEnabled' => __( 'Facebook oEmbeds are turned on', 'custom-twitter-feeds' ),
				'enable' => __( 'Enable', 'custom-twitter-feeds' ),
				'disable' => __( 'Disable', 'custom-twitter-feeds' ),
				'whatAreOembeds' => __( 'What are oEmbeds?', 'custom-twitter-feeds' ),
				'whatElseOembeds' => __( 'What else can the Instagram Feed plugin do?', 'custom-twitter-feeds' ),
				'whenYouPaste' => __( 'When you paste a link to a Instagram or Instagram post in WordPress, it automatically displays the post instead of the URL. That is called an oEmbed.', 'custom-twitter-feeds' ),
				'dueToRecent' => __( 'Due to recent API changes from Instagram, WordPress cannot automatically embed your posts.', 'custom-twitter-feeds' ),
				'however' => __( 'However, we have added this feature to Smash Balloon to make sure your oEmbeds keep working.', 'custom-twitter-feeds' ),
				'justEnable' => __( 'Just enable it above, and all your existing and new embeds should work automatically, no other input required.', 'custom-twitter-feeds' ),
				'displayACompletely' => __( 'Display a completely customizable Instagram Feed with tons of features', 'custom-twitter-feeds' ),
				'createACustom' => __( 'Create a custom styled feed of your Instagram posts which integrates seamlessly with your WordPress theme.', 'custom-twitter-feeds' ),
            ),
            'images' => array(
                ...
                'image1_2x' => CTF_PLUGIN_URL . 'admin/assets/img/oembeds-image-1@2x.png',
                'image2_2x' => CTF_PLUGIN_URL . 'admin/assets/img/oembeds-image-2@2x.png',
                'image3_2x' => CTF_PLUGIN_URL . 'admin/assets/img/oembeds-image-3@2x.png',
                'image4_2x' => CTF_PLUGIN_URL . 'admin/assets/img/oembeds-image-4@2x.png',
			),
			'modal' => array(
				'title' => __( 'Enable Instagram oEmbeds', 'custom-twitter-feeds' ),
				'description' => __( 'To enable Instagram oEmbeds our Instagram Feed plugin is required. Click the button below to Install it and enable Instagram oEmbeds.', 'custom-twitter-feeds' ),
				'install' => __( 'Install Plugin', 'custom-twitter-feeds' ),
				'activate' => __( 'Activate Plugin', 'custom-twitter-feeds' ),
				'cancel' => __( 'Cancel', 'custom-twitter-feeds' ),
				'instaIcon' => CTF_PLUGIN_URL . 'admin/assets/img/instagram-color-icon.svg',
		...
		if ( ! empty( $newly_retrieved_oembed_connection_data['access_token'] ) ) {
			...
			update_option( 'ctf_oembed_token', $newly_retrieved_oembed_connection_data );
	...
	public static function get_connection_url() {
		...
		$admin_url_state = admin_url( 'admin.php?page=ctf-oembeds-manager' );
		//If the admin_url isn't returned correctly then use a fallback
		if( $admin_url_state == '/wp-admin/admin.php?page=ctf-oembeds-manager' ){
	...
	public static function maybe_connection_data( $saved_access_token_data ) {
		...
		if ( $screen->id !== 'twitter-feed_page_ctf-oembeds-manager') {
			return false;
		}
		...
		} if ( isset( $_GET['ctf_access_token'] ) ) {
			$access_token = $_GET['ctf_access_token'];

			$return = [];

			$valid_new_access_token = ! empty( $access_token ) && strlen( $access_token ) > 20 && $saved_access_token_data !== $access_token ? sanitize_text_field( $_GET['ctf_access_token'] ) : false;
	...
	public function instagram_oembed_enabled() {
		$ctf_oembed_token = get_option( 'ctf_oembed_token' );
		if ( isset( $ctf_oembed_token['access_token'] ) && isset( $ctf_oembed_token['disabled'] ) && ! $ctf_oembed_token['disabled'] ) {
			return true;
		}
		return false;
	}
	...
	public function oembeds_manager(){
		return \TwitterFeed\Admin\CTF_View::render( 'oembeds.index' );
	}
}