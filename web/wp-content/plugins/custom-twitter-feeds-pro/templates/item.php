...
if ( $feed_options['linktexttotwitter'] ) {
	$tweet_text_start = '<a class="ctf-tweet-text-link" href="' . esc_url( 'https://twitter.com/' . $author_screen_name . '/status/' .$post_id ) . '" target = "_blank" rel = "noopener noreferrer">';
	$tweet_text_end = '</a>';
}
?>
...
<div class="<?php echo esc_attr( $tweet_classes ) ?>" id="ctf_<?php echo esc_attr( $id_post_id ) ?>" style="<?php echo esc_attr( $feed_options['tweetbgcolor'] ) ?>">
    ...
    <?php if ( ctf_show( 'twitterlink', $feed_options ) ) : ?>
        <a href="<?php echo esc_url( 'https://twitter.com/' . $author_screen_name . '/status/' .$post_id ) ?>" class="ctf-twitterlink" style="<?php echo esc_attr( $twitter_link_styles ) ?>" target="_blank">
