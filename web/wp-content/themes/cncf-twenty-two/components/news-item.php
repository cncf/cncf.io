...
<a class="post-archive__link" href="<?php echo esc_url( $link_url ); ?>" title="<?php the_title_attribute(); ?>">
		...
		if ( has_post_thumbnail() ) {
			Lf_Utils::display_responsive_images( get_post_thumbnail_id(), 'newsroom-media-coverage', '600px', 'post-archive__image', 'lazy', the_title_attribute() );
		} else {
	...
	?>
		...
		<a class="post-archive__link" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
			...
			if ( has_post_thumbnail() && $is_featured ) {
				...
				Lf_Utils::display_responsive_images( get_post_thumbnail_id(), 'newsroom-700', '700px', 'post-archive__image', 'lazy', the_title_attribute() );

			} elseif ( has_post_thumbnail() ) {
				// display smaller news image.
				Lf_Utils::display_responsive_images( get_post_thumbnail_id(), 'newsroom-400', '400px', 'post-archive__image', 'lazy', the_title_attribute() );

			} else {
				// show generic.
				$site_options = get_option( 'lf-mu' );
				Lf_Utils::display_responsive_images( $site_options['generic_thumb_id'], 'newsroom-400', '400px', 'post-archive__image', 'lazy', the_title_attribute() );
			}
			?>
		</a>
		</div>
